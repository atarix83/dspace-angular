...
export class ConnectArrayDirective extends ControlContainer implements OnInit {
  ...
  constructor(
    ...
    this.valueAccessor = selectValueAccessor(this as any, valueAccessors) || this.simpleAccessor();
  ...
  private resetState(state: any) {
    ...
    const iterable = FormState.get(state, this.connection.path.concat(this.path));

    let index = 0;

    for (const value of iterable) {
      let viewRef = this.viewContainerRef.length > index
