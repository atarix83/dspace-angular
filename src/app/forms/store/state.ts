...
export abstract class FormState {
  static traverse<FormState>(state: FormState, path: string[], fn?: TraverseCallback) {
    let deepValue = state;
  ...
  static get<State>(state: FormState, path: string[]): any {
    return FormState.traverse(state, path);
  }

  static assign<FormState>(state: FormState, path: string[], value?: any) {
    const operations = FormState.inspect(state);
    ...
    FormState.traverse(root, path,
      (parent, key: number | string, remainingPath: string[], innerValue?) => {
        const parentOperations = FormState.inspect(parent);

        if (innerValue) {
          const innerOperations = FormState.inspect(innerValue);
          ...
          const getProbableType = (typeKey: string | number) => {
            ...
            return typeof typeKey === 'number'
              ? []
              : Array.isArray(typeKey)
                ? ImmutableMap()
                : {};
          };
  ...
  static inspect<K>(object: K): Operations<K> {
    const metaOperations = (update: (...T) => void, merge: (...T) => void, clone?: (...T) => void) => {
      const operations = {
      ...
      return metaOperations(
        ...
        (parent: any, key: number | string, value: K) => {
          if (key != null) {
            ...
            parent.clear();
            m.forEach((innerValue, index) => parent.set(index, innerValue));
            return parent;
          }
        },
        ...
        (parent: Map<string, any>, _: any, value: K) => {
          ...
          m.forEach((innerValue, key) => parent.set(key, innerValue));
          return parent;
        },
      ...
      return metaOperations(
        ...
        (parent: any, key: number, value: K) => {
          if (key != null) {
            ...
            s.forEach((tempValue, index) => parent.set(index, tempValue));
            s.clear();
            return parent;
          }
        },
